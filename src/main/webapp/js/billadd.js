...
$(function(){
	...
	billCode.on("blur",function(){
		...
		$.ajax({
			...
			data: {
				method: "existBillCode",
				billCode:billCode.val()
			},
			dataType: "json",
			success : function(data) {
			    if(data.billCode == "empty") {
			    	validateTip(billCode.next(),{"color":"red"}, imgNo+" 请输入订单编码",false);
				} else if(data.billCode == "exist") {
					validateTip(billCode.next(),{"color":"red"}, imgNo+" 该订单编码已存在",false);
				} else {
					validateTip(billCode.next(),{"color":"green"}, imgYes+" 该订单编码可以使用",true);
				}
			},
			error:function(data) {
